...
class classWriter extends HTMLWriter
{
            ...
            if ($classes) {
                ...
                foreach ($classes as $name => $class) {
                    ...
                    if ($class->isInterface()) {
                        echo '<h2>Interface ', $class->name(), "</h2>\n\n";
                    } else {
                        echo '<h2>Class ', $class->name(), "</h2>\n\n";
                    }
                    ...
                    if ($constants) {
                        echo '<table id="summary_field" class="table table-striped table-hover">', "\n";
                        echo '<thead><tr><th colspan="2">Constant Summary</th></tr></thead>', "\n";
                        foreach ($constants as $field) {
                            $textTag =& $field->tags('@text');
                            echo "<tr>\n";
                            echo '<td class="type">', $this->styleModifiers($field->modifiers(FALSE)), ' ', $field->typeAsString(), "</td>\n";
                    ...
                    if ($fields) {
                        echo '<table id="summary_field" class="table table-striped table-hover">', "\n";
                        echo '<thead><tr><th colspan="2">Field Summary</th></tr></thead>', "\n";
                    ...
                    if ($constructor) {
                        echo '<table id="summary_constructor" class="table table-striped table-hover">', "\n";
                        echo '<thead><tr><th colspan="2">Constructor Summary</th></tr></thead>', "\n";
                        $textTag =& $constructor->tags('@text');
                        echo "<tr>\n";
                        echo '<td class="type">', $this->styleModifiers($constructor->modifiers(FALSE)), ' ', $constructor->returnTypeAsString(), "</td>\n";
                        echo '<td class="description">';
                    ...
                    if ($methods) {
                        echo '<table id="summary_method" class="table table-striped table-hover">', "\n";
                        echo '<thead><tr><th colspan="2">Method Summary</th></tr></thead>', "\n";
                        foreach ($methods as $method) {
                            $textTag =& $method->tags('@text');
                            echo "<tr>\n";
                            echo '<td class="type">', $this->styleModifiers($method->modifiers(FALSE)), ' ', $method->returnTypeAsString(), "</td>\n";
                            echo '<td class="description">';
                    ...
                    if ($constants) {
                        ...
                        foreach ($constants as $field) {
                            ...
                            echo '<code class="signature">', $this->styleModifiers($field->modifiers()), ' ', $field->typeAsString(), ' <strong>';
                            if (is_null($field->constantValue())) echo '$';
                    ...
                    if ($constructor) {
                        ...
                        echo '<code class="signature">', $this->styleModifiers($constructor->modifiers()), ' ', $constructor->returnTypeAsString(), ' <strong>';
                    ...
                    if ($methods) {
                        ...
                        foreach ($methods as $method) {
                            ...
                            echo '<code class="signature">', $this->styleModifiers($method->modifiers()), ' ', $method->returnTypeAsString(), ' <strong>';
    ...
    /**
     * Wraps reserved words in styles span
     * @param str $modifiers
     * @return str
     */
    public function styleModifiers($modifiers)
    {
        ...
        foreach ($modifier_classes as $modifier => $class) {
            $modifiers = str_replace($modifier, "<span class=\"label label-$class\">$modifier</span>", $modifiers);
        }
        return $modifiers;
    }
