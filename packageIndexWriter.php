...
class packageIndexWriter extends HTMLWriter
{
    ...
    public function packageIndexWriter(&$doclet)
    {
        ...
        echo '<thead><tr><th colspan="2" class="title"><h3>Namespaces</h3></th></tr></thead>'."\n".'<tbody>';
        $packages =& $rootDoc->packages();
        ksort($packages);
        foreach ($packages as $name => $package) {
            $textTag =& $package->tags('@text');
            echo '<tr><td class="name"><a href="'.$package->asPath().'/package-summary.html">'.$package->name().'</a></td>';
            echo '<td class="description">'.$textTag.strip_tags($this->_processInlineTags($textTag, TRUE), '<a><b><strong><u><em>').'</td></tr>'."\n";
        }
        echo '</tbody></table>'."\n\n";
