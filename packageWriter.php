...
class packageWriter extends HTMLWriter
{
    ...
    public function packageWriter(&$doclet)
    {
        ...
        foreach ($packages as $packageName => $package) {
            ...
            echo "<h1>{$package->name()} <small>namespace</small></h1>\n\n";
            ...
            if ($classes) {
                ksort($classes);
                echo '<table class="title table table-striped table-hover">', "\n";
                echo '<thead><tr><th colspan="2" class="title">Class Summary</th></tr></thead>', "\n";
            ...
            if ($interfaces) {
                ksort($interfaces);
                echo '<table class="title table table-striped table-hover">'."\n";
                echo '<thead><tr><th colspan="2" class="title">Interface Summary</th></tr></thead>'."\n";
            ...
            if ($traits) {
                ksort($traits);
                echo '<table class="title table table-striped table-hover">'."\n";
                echo '<thead><tr><th colspan="2" class="title">Trait Summary</th></tr></thead>'."\n";
            ...
            if ($exceptions) {
                ksort($exceptions);
                echo '<table class="title table table-striped table-hover">'."\n";
                echo '<thead><tr><th colspan="2" class="title">Exception Summary</th></tr></thead>'."\n";
            ...
            if ($functions) {
                ksort($functions);
                echo '<table class="title table table-striped table-hover">', "\n";
                echo '<thead><tr><th colspan="2" class="title">Function Summary</th></tr></thead>', "\n";
            ...
            if ($globals) {
                ksort($globals);
                echo '<table class="title table table-striped table-hover">', "\n";
                echo '<thead><tr><th colspan="2" class="title">Global Summary</th></tr></thead>', "\n";
            ...
            $textTag =& $package->tags('@text');
            if ($textTag) {
                echo '<h2>Namespace ', $package->name(), " Description</h2>\n\n";
            ...
            if ($displayTree) {
                ...
                echo '<h2>Class Hierarchy for Package ', $package->name(),'</h2>';
    ...
    public function _displayTree($tree, $parent = NULL)
    {
        ...
        foreach ($tree as $name => $element) {
            if ($element->superclass() == $parent) {
                if ($outputList) echo "<ul class=\"list-unstyled list-tree\">\n";
